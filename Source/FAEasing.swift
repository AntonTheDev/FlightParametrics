...
public enum FPEasing : Equatable {
    case linear
    case smoothStep
    case smootherStep
    case inAtan
    ...
    public func parametricProgress(_ p : CGFloat) -> CGFloat {
        ...
        case .inBounce:
            return 1.0 - FPEasing.outBounce.parametricProgress(1.0 - p)
        case .outBounce:
            if(p < 4.0/11.0) {
        ...
        case .inOutBounce:
            if(p < 0.5) {
                return 0.5 * FPEasing.inBounce.parametricProgress(p * 2.0);
            } else{
                return 0.5 * FPEasing.outBounce.parametricProgress(p * 2.0 - 1.0) + 0.5;
            }
        case .outInBounce:
            if(p < 0.5) {
                return 0.5 * FPEasing.outBounce.parametricProgress(p / 2.0);
            } else{
                return 0.5 * FPEasing.inBounce.parametricProgress(p * 2.0 - 1.0) + 0.5;
            }
        }
    }
...
public func ==(lhs : FPEasing, rhs : FPEasing) -> Bool {
    switch lhs {
    case .linear:
